...
export default function AdminQikinkProducts() {
  ...
  const removeColor = (i:number)=>{
    const confirmDelete = confirm("⚠️ Kya aap sure ho? Yeh variant delete ho jayega");
  ...
  const saveProduct = async()=>{
    ...
    if (!name || !qikinkId || !sku || !printTypeId || !category) {
      alert("⚠️ Basic details fill karo");
      return;
    }
    ...
    for (let i = 0; i < variations.length; i++) {
      ...
      if (!v.color || !v.basePrice || !v.sellPrice) {
        alert(`⚠️ Variant ${i+1} ka data incomplete hai`);
        return;
      }

      if (!v.mainImage) {
        alert(`⚠️ Variant ${i+1} ka main image missing hai`);
        return;
      }
      ...
      for (let j = 0; j < v.sizes.length; j++) {
        ...
        if (!s.size || !s.price || !s.stock) {
          alert(`⚠️ Variant ${i+1} → Size ${j+1} fill karo`);
          return;
        }
      }
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 p-6">
      ...
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-3xl shadow-2xl">
        ...
        <button
          onClick={saveProduct}
          disabled={loading}
          className="btn-blue w-full mt-6 flex justify-center gap-2"
        >
          {loading ? "Saving..." : <><PlusCircle/> Add Product</>}
        </button>
