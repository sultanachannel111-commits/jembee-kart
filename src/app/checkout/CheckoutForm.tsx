...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(auth, async (u) => {
      ...
      // 🛒 Items Loading Logic (Restored Buy Now & Cart)
      ...
      if (buyNow) {
        try {
          ...
          setItems([{
            ...
            qty: Number(parsed.quantity || parsed.qty) || 1,
            sellerId: parsed.sellerId || "" // ✅ Seller ID restore
          }]);
        ...
        unsubscribeCart = onSnapshot(ref, (snap) => {
          ...
          snap.forEach(docSnap => {
            ...
            data.push({
              ...
              sellerId: d.sellerId || "" // ✅ Seller ID sync
            });
          });
          setItems(data);
        });
  ...
  const totalCommission = profit > 0 ? Math.round(profit * 0.50) : 0;
  ...
  const handlePayment = async () => {
    ...
    try {
      const orderData = { 
        ...
        sellerRef: items[0]?.sellerId || "", // ✅ Direct linked with items
        address, 
        paymentMethod,
        orderStatus: "PLACED",
          ...
          {address ? (
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                {address.street || address.address}, {address.city}, {address.state} - {address.zip || address.pincode}
              </p>
            </div>
          ) : (
